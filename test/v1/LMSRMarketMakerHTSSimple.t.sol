...
contract LMSRMarketMakerSimpleTest is OracleSetUP, ExpiryHelper, KeyHelper, HederaTokenService {
    ...
    int64 constant INITIAL_SUPPLY = int64(int256(1_000 * (10 ** uint64(DECIMALS))));
    ...
    function _executeTradingSequence() private {
        ...
        int64[][] memory amounts = _getTradeAmounts();
        ...
        for (uint256 i = 0; i < traders.length; i++) {
            ...
            int256 responseCode = approve(
                outcomeTokenAddresses[0], address(marketMaker), uint256(1_000_000 * int256(10 ** uint256(uint64(DECIMALS))))
            );
            responseCode = approve(
                outcomeTokenAddresses[1], address(marketMaker), uint256(1_000_000 * int256(10 ** uint256(uint64(DECIMALS))))
            );
    ...
    function _getTradeAmounts() private pure returns (int64[][] memory amounts) {
        ...
        amounts[0][0] = 300 * int64(uint64(10) ** uint64(DECIMALS));
        amounts[0][1] = 4500 * int64(uint64(10) ** uint64(DECIMALS));
        ...
        amounts[4][0] = 1 * int64(uint64(10) ** uint64(DECIMALS));
        amounts[4][1] = 0 * int64(uint64(10) ** uint64(DECIMALS));
