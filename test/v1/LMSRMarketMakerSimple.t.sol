...
contract LMSRMarketMakerSimpleTest is OracleSetUP, ExpiryHelper, KeyHelper, HederaTokenService {
    ...
    struct UserInfo {
        uint256 shares;               // текущее количество шейров у пользователя
        uint256 rewardDebt;           // snapshot accRewardPerShare на момент последнего апдейта
        uint256 pendingRewards;       // уже «зарезервированные» (но ещё не выплаченные) дивиденды
    }
